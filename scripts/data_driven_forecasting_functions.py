        ...
        for i in range(iterations):
            ...
            for s in cols:
                ...
                if n > 0:
                    ...
                    gapfilled_data = qm_gap_filling(original_data, artificial_gaps_data, window_days, min_obs_corr, min_obs_cdf, min_corr, flag=1)[0]
...
def regime_classification(streamflow_obs_path, start_water_year, max_gap_days, flag):
    ...
    streamflow_data_ds = xr.open_dataset(streamflow_obs_path)

    # Create Pandas GeoDataFrame to save circular statistics
    basins_regimes_df = streamflow_data_ds.Station_ID.to_dataframe()
    ...
    basins_regimes_gdf = basins_regimes_gdf.drop(columns=['lat','lon','Station_ID'])
    ...
    for s in streamflow_data_ds.Station_ID.values:
        ...
        # we also run some linear interpolation to maximize the number of values available to calculate the peak flow statistcs
...
# def stations_basin_map(basin, basin_id, stations, dem_dir, flag, buffer_km=0):
def stations_basin_map(basins, basin_id, SWE_stations, P_stations, flag, buffer_km=0):
    ...
    - basin_id: String of the basin id to plot
    - SWE_stations: Pandas GeoDataFrame of SWE stations to plot
    - P_stations: Pandas GeoDataFrame of SWE stations to plot
    - dem_dir: String of the path to DEMs
